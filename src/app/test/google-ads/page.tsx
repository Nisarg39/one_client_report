...
import { getGoogleAdsTestData } from './test-action';
...
export default function GoogleAdsTestPage() {
    ...
    const handleFetch = async (campaignId?: string) => {
        ...
        setError(null);
        try {
            const result = await getGoogleAdsTestData(selectedClientId, campaignId || '');
            if (result.success) {
                setData(result.data);
            } else {
    ...
    return (
        <div className="p-8 max-w-6xl mx-auto bg-[#1a1a1a] min-h-screen text-white">
            <h1 className="text-3xl font-bold mb-8">Google Ads Debugger</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                <div className="bg-[#252525] p-6 rounded-xl border border-gray-800">
                    ...
                    <button
                        ...
                        className="w-full bg-[#FBBC05] hover:bg-[#F9AB00] text-black font-bold py-2 px-4 rounded disabled:opacity-50"
                    >
                ...
                <div className="space-y-10">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div className="bg-[#252525] border border-yellow-500/20 p-5 rounded-xl text-center shadow-lg">
                                        ...
                                        {data.campaigns?.map((c: any) => (
                                            <tr
                                                ...
                                                className={`
                                                    ...
                                                    ${selectedCampaignId === c.id ? 'bg-yellow-600/20 border-l-4 border-yellow-500' : 'hover:bg-white/5'}
                                                `}
                                                onClick={() => {
                                            ...
                                            >
                                                ...
                                                <td className="p-5 text-right font-mono text-yellow-500 text-base">
                                                    {data.metrics.currency} {typeof c.spend === 'number' ? c.spend.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : c.spend}
                                                </td>
                                                ...
                                                <td className="p-5">
                                                    <span className={`px-2.5 py-1 rounded text-[9px] uppercase font-black tracking-tighter ${c.status === 'ENABLED' ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-gray-800 text-gray-500 border border-gray-700/50'
                                                        }`}>
                                                        {c.status}
                                                    </span>
                                                </td>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 pt-10 border-t border-gray-800">
                        <div>
                            ...
                            <pre className="bg-[#0f0f0f] p-6 rounded-2xl border border-gray-800 overflow-auto max-h-[500px] text-[11px] font-mono text-yellow-300/40 leading-relaxed shadow-inner">
