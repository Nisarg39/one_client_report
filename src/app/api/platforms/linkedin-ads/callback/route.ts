/**
 * LinkedIn Ads OAuth Callback Handler
 *
 * Handles the OAuth callback from LinkedIn Ads
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Handle OAuth error
    if (error) {
      console.error('[LinkedIn Ads OAuth] Error:', error, errorDescription);
    ...
    return NextResponse.redirect(
      new URL(`${returnPath}?success=linkedin-ads`, request.url)
    );
  } catch (error: any) {
